    ...
    return {
      ...
      speaker:
        kind === "dialogue"
          ...
          : undefined,
      text:
        kind === "dialogue" || kind === "story-caption"
          ? expectString(record.text, `${fieldName}[${index}].text`)
          : parseOptionalString(record.text, `${fieldName}[${index}].text`),
      autoAdvanceAfterMs: parseOptionalPositiveNumber(
        record.autoAdvanceAfterMs,
        `${fieldName}[${index}].autoAdvanceAfterMs`
      ),
      requireConfirm: parseOptionalBoolean(record.requireConfirm, `${fieldName}[${index}].requireConfirm`)
    };
  });
}
...
function parseFreePhase(record: Record<string, unknown>, index: number): LevelPhaseDefinition {
  return {
    ...
    hudPrompt: parseOptionalString(record.hudPrompt, `phases[${index}].hudPrompt`),
...
function parsePracticePhase(record: Record<string, unknown>, index: number): PracticePhaseDefinition {
  return {
    ...
    hudPromptTemplate: parseOptionalString(record.hudPromptTemplate, `phases[${index}].hudPromptTemplate`),
...
function parseOptionalPositiveNumber(value: unknown, fieldName: string): number | undefined {
  ...
  return expectPositiveNumber(value, fieldName);
}
